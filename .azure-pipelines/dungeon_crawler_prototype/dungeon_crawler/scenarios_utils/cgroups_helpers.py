...
def check_for_unexpected_processes_for_agent(processes):
    # processes is a list of [(pid, cmdline), (pid, cmdline)]
    daemon_pid = -1
    agent_pid = -1

    for pid, cmd in processes:
        if re.match(DAEMON_CMDLINE_PATTERN, cmd):
            daemon_pid = pid
            continue

        if re.match(AGENT_CMDLINE_PATTERN, cmd):
            agent_pid = pid
            continue
...
def verify_agent_cgroup_assigned_correctly_systemd(service_status):
    ...
    processes = parse_processes_from_systemctl_status(service_status)
    check_for_unexpected_processes_for_agent(processes)
